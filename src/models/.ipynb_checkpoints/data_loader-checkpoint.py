...
class Batch(object):
    ...
    def __init__(self, data=None, device=None, is_test=False):
        ...
        if data is not None:
            ...
            ent_list = [torch.LongTensor(x[5]).to(device) for x in data]
...
class DataIterator(object):
    ...
    def preprocess(self, ex, is_test):
        ...
        rel_len = len(rel_list)
        ent_index = dict(zip(ent_list, list(range(ent_len))))
        rel_index = dict(zip(rel_list, list(range(rel_len))))

        adjsize = ent_len + rel_len + 1
        ...
        if ent_len > 0:
            ...
            for y in spo:
                ...
                s_index = ent_index[s]
                o_index = ent_index[o]
                p_index = rel_index[p] + ent_len
                adj[s_index, o_index] = 1
                adj[o_index, p_index] = 1
        else:
            adj = torch.ones(1, 1)
            rel_list = [46]
        ...
        if(is_test):
            return src, tgt, segs, clss, src_sent_labels, ent_list, rel_list, adj, src_txt, tgt_txt
        else:
