    ...
    def forward(self, key, value, query, mask=None,
        ...
        if layer_cache is not None:
            if type == "self":
                ...
                query = self.linear_query(query)
                if layer_cache is not None:
                    if layer_cache["graph_keys"] is None:
                        key, value = self.linear_keys(key), \
                                     self.linear_values(value)
                        key = shape(key)
                        value = shape(value)
                    else:
                        key, value = layer_cache["graph_keys"], \
                                     layer_cache["graph_values"]
                    layer_cache["graph_keys"] = key
                    layer_cache["graph_values"] = value
                else:
                    key, value = self.linear_keys(key), \
                                 self.linear_values(value)
                    key = shape(key)
        ...
        # if type == "graph_context":
        #     topk_index = torch.topk(scores,)
        #     scores.masked_fill()
