...
class Translator(object):
    ...
    def _report_rouge(self, gold_path, can_path):
        ...
        results_dict = test_rouge(self.args.temp_result_dir, can_path, gold_path)
        return results_dict
    ...
    def _fast_translate_batch(self,
        ...
        # graph and beam extend
        gents, emask = self.model.graph_encoder(batch, self.model.bert.model.embeddings)
        gents = tile(gents, beam_size, 0)
        emask = tile(emask, beam_size, 0)
        #
        if self.copy == True:
            ent_top_vec = self.model.bert(batch.ent_src, batch.ent_seg_ids, batch.mask_ent_src)
            ent_top_vec = tile(ent_top_vec, beam_size, dim=0)
            ent_src = tile(batch.ent_src, beam_size, dim=0)
        ...
        for step in range(max_length):
            # b*beam_size x 1 -> 1 x b*beam_size
            decoder_input = alive_seq[:, -1].view(1, -1)

            # Decoder forward.
            # b*beam_size x 1(normal: b x t)
            decoder_input = decoder_input.transpose(0,1)

            # dec_out: b*beam_size x 1 x h

            # layer_cache will cache and concat former information
            dec_out, dec_states, src_context, graph_context = self.model.decoder(decoder_input, src_features, dec_states,
                                                     step=step, gents=gents, emask=emask)
            # Generator forward.
            if not self.copy:
                # graph etc will not be used
                log_probs, _ = self.model.generator.forward(dec_out.squeeze(1), src, src_features)
            else:
                log_probs, _ = self.model.generator.forward(dec_out, ent_src, ent_top_vec)
            ...
            if(self.args.block_trigram):
                ...
                if(cur_len>3):
                    for i in range(alive_seq.size(0)):
                        ...
                        if fail:
                            curr_scores[i] = -10e20
            # TODO b*beam_size x vocab -> b x beam_size*vocab
            # Overall optimum under three conditions
            ...
            if self.copy == True:
                ent_top_vec = ent_top_vec.index_select(0, select_indices)
                ent_src = ent_src.index_select(0, select_indices)
            gents = gents.index_select(0, select_indices)
            emask = emask.index_select(0, select_indices)
            dec_states.map_batch_fn(
