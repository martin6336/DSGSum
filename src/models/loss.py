...
class LossComputeBase(nn.Module):
    ...
    def monolithic_compute_loss(self, batch, output,
                                context_src, context_graph, src, top_vec, copy=False):
        """
        ...
        """
        shard_state = self._make_shard_state(batch, output, context_src, src, top_vec, copy)

        # TODO
        # _, batch_stats = self._compute_loss(batch, **shard_state)
        loss, batch_stats, _, _ = self._compute_loss(batch, **shard_state)
    ...
    def sharded_compute_loss(self, batch, output,
        ...
        for shard in shards(shard_state, shard_size):
            count+=1
            loss, stats, scores, copy_v = self._compute_loss(batch, **shard)
            loss.div(float(normalization)).backward()
            batch_stats.update(stats)

        return batch_stats, copy_v
...
class NMTLossCompute(LossComputeBase):
    ...
    def __init__(self, generator, symbols, vocab_size,
        ...
        self.sparse = not isinstance(generator.voc_gen[1], nn.LogSoftmax)
    ...
    def _make_shard_state(self, batch, output, context_src, src, top_vec, copy):
        # context_src, context_graph, src, top_vec
        if copy:
            return {
                "output": output,
                "target": batch.tgt[:,1:],
    ...
    def _compute_loss(self, batch, output, target, src=None, top_vec=None):
        # bottled_output = self._bottle(output)
        scores, copy_v = self.generator(output, src, top_vec)
        ...
        return loss, stats, scores, copy_v
...
def shards(state, shard_size, eval_only=False):
    ...
    if eval_only:
        ...
        # 这里variable必须要用到，否则报错说只适用scalar
