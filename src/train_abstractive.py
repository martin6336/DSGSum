...
def visual_abs(args, device_id):
    timestep = 0
    if (args.test_all):
        # cp_files = sorted(glob.glob(os.path.join(args.model_path, 'model_step_*.pt')))
        # cp_files.sort(key=os.path.getmtime)
        cp_files = args.test_from
        xent_lst = []
        # for i, cp in enumerate(cp_files):
        # step = int(cp.split('.')[-2].split('_')[-1])
        cp = args.test_from
        ...
        xent = visual_validate(args, device_id, cp, step)


def visual_validate(args, device_id, pt, step):
    device = "cpu" if args.visible_gpus == '-1' else "cuda"
    if (pt != ''):
        test_from = pt
    else:
    ...
    trainer = build_trainer(args, device_id, model, None, valid_loss)
    stats = trainer.attention_validate(valid_iter, step, tokenizer)
    return stats.xent()
